                                     ...
                                     Bwx& aUrl)
{
    ...
    // Obtain the version of the platform GLibc.
    const TChar *platformVersion = gnu_get_libc_version();
    ...
    try
    {
        for (;;)
        {
            ...
            if (ReadValue(readerUntil, jsonKey) == Brn(xstr(TARG_ARCH)))
            {
                ...
                if (isOlderVersion(platformVersion,
                                   (const TChar *)(minVer.Ptr())))
                {
                    continue;
                }
                ...
                if (isOlderVersion(CURRENT_VERSION,
                                   (const TChar *)(version.Ptr())))
                {
                    jsonKey = Brn("uri");
