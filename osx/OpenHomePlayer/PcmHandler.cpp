...
void OsxPcmProcessor::fillBuffer(AudioQueueBufferRef inBuffer)
{
    ...
    while((iBytesToRead > 0) && iOutputActive)
    {
        ...
        // If the current message wont fit in the audio buffer in it's
        // entirety requeue and exit. It to be processed for the next audio
        // buffer.
        //
        // No attempt is made to 'split' the buffer as we must ensure the
        // split occurs on a frame boundary.
        //
        // The data may be of the current audio queue format or 32 bit pcm
        // generated by the ramper. There is no way of telling at this stage.
